...
function filename(clientName: string) { return `${clientName.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "") || "client"}-UX-Audit.pdf`; }
...
async function exportPdf(audit: any) {
  ...
  let html = audit.report_html;
  for (const page of audit.pages || []) {
    const assetUrl = `/api/audit-assets/${encodeURIComponent(audit.id)}?pageId=${encodeURIComponent(page.id)}`;
    html = html.replaceAll(`src="${assetUrl}"`, `src="${page.screenshot}"`);
  }
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({ html, options: { printBackground: true, margin: { top: "0", right: "0", bottom: "0", left: "0" }, preferCSSPageSize: true } }),
    signal: AbortSignal.timeout(60000),
  });
  if (!response.ok) { const body = await response.text().catch(() => ""); throw new Error(`PDF export service returned HTTP ${response.status}${body ? `: ${body.slice(0, 240)}` : ""}`); }
...
export async function GET(request: Request) {
  try {
    ...
    if (params.get("format") === "pdf") {
      ...
      return new Response(pdf, { status: 200, headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${filename(audit.client_name)}"`, "Cache-Control": "no-store" } });
    }
