...
async function extractBrandPalette(dataUri: string) {
  ...
  const { data, info } = await sharp(input).resize(70, 70, { fit: "cover" }).removeAlpha().raw().toBuffer({ resolveWithObject: true });
...
function addText(slide: any, text: string, x: number, y: number, w: number, h: number, opts: Record<string, unknown> = {}) {
  slide.addText(text, { x, y, w, h, margin: 0, fontFace: "Aptos", color: "171717", fit: "shrink", ...opts });
}

function addScreenshot(slide: any, screenshot: string, width: number, height: number, boxX: number, boxY: number, boxW: number, boxH: number, marker?: { x: number; y: number }, markerColor = "E51E7A", markerLabel?: string) {
...
function addFooter(slide: any, clientName: string, accent: string, page: number) {
...
export async function GET(request: Request) {
  try {
    ...
    slide.addShape(PptxGenJS.ShapeType.roundRect, { x: 0.75, y: 5.72, w: 3.0, h: 0.62, fill: { color: palette.secondary }, line: { color: palette.secondary } });
    ...
    stats.forEach(([value, label], i) => {
      const x = 0.65 + i * 3.02;
      slide.addShape(PptxGenJS.ShapeType.roundRect, { x, y: 1.72, w: 2.7, h: 1.25, fill: { color: i === 0 ? palette.primary : "FFFFFF" }, line: { color: i === 0 ? palette.primary : "E1DFD7", width: 0.8 } });
    ...
    slide.addShape(PptxGenJS.ShapeType.roundRect, { x: 8.25, y: 1.55, w: 4.35, h: 4.95, fill: { color: "FFFFFF" }, line: { color: "E1DFD7", width: 0.8 } });
    addText(slide, "What this audit covers", 8.62, 1.95, 3.5, 0.3, { fontSize: 13, bold: true });
    addText(slide, "Visual hierarchy\nNavigation & interaction\nLanguage & clarity\nConversion friction\nUX research principles\nBusiness funnel impact", 8.62, 2.55, 3.25, 2.3, { fontSize: 11, color: "4D4C47", valign: "top" });
    addText(slide, "The interface was reviewed from the supplied screenshot with evidence anchored to visible UI regions.", 8.62, 5.35, 3.25, 0.72, { fontSize: 9, color: "686860", italic: true, breakLine: true });
    addFooter(slide, audit.client_name, palette.primary, slideNo);
    ...
    findings.forEach((f: any, index: number) => {
      ...
      slide.addShape(PptxGenJS.ShapeType.roundRect, { x: 0.65, y: 3.28, w: 5.15, h: 1.1, fill: { color: "FFFFFF" }, line: { color: "E1DFD7", width: 0.7 } });
    ...
    slide.addShape(PptxGenJS.ShapeType.roundRect, { x: 9.05, y: 5.95, w: 3.55, h: 0.62, fill: { color: palette.primary }, line: { color: palette.primary } });
