...
export default function ClientReportPage({ params }: { params: { auditId: string } }) {
  ...
  return <main className="reportShell">
    <header className="reportToolbar"><a href="/knowledge-base" className="back">← Knowledge base</a><div className="toolbarCenter"><strong>{clientName}</strong><span>Client presentation</span></div><button onClick={() => void exportPdf()} disabled={loading || exporting || !html}>{exporting ? "Preparing PDF…" : "Export PDF ↓"}</button></header>
