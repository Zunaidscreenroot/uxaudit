...
function EvidenceMap({ page }: { page: AuditPage }) {
  ...
  return <div className="screenshotStage">
    <img className="fullPageScreenshot" src={page.screenshot} width={width} height={height} alt={`Full-page screenshot of ${page.url} with verified evidence regions`} />
  </div>;
}
...
export default function Home() {
  ...
  return <main className="shell">
    ...
    {result && page && <section className="results" aria-live="polite">
      ...
      <div className="card screenshotCard"><div className="sectionHeader"><div><div className="muted">VISUAL EVIDENCE</div><h3>Verified landing page screenshot</h3></div><span className="pill">{evidence.length} verified highlighted region{evidence.length === 1 ? "" : "s"}</span></div><p className="screenshotHint">The final screenshot preserves the captured page at its native aspect ratio. Yellow regions are baked into the image only after AI verification and correction.</p><EvidenceMap page={page} /></div>
