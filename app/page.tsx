...
export default function Home() {
  ...
  const evidence = useMemo(
    () => page?.findings.flatMap((finding) => finding.evidence.map((item) => ({ ...item, finding }))) ?? [],
    [page],
  );
  ...
  const summary = page
    ? `${page.findings.length} visually evidenced UX finding${page.findings.length === 1 ? "" : "s"} identified across the landing page.`
    : "";
  ...
  return <main className="shell">
    ...
    {result && page && <section className="results" aria-live="polite">
      ...
      <div className="card screenshotCard"><div className="sectionHeader"><div><div className="muted">VISUAL EVIDENCE</div><h3>Landing page screenshot</h3></div><span className="pill">{evidence.length} highlighted region{evidence.length === 1 ? "" : "s"}</span></div><p className="screenshotHint">Yellow boxes show the regions referenced by the analysis. Region numbers correspond to the explanations below.</p><div className="screenshotStage"><img className="auditScreenshot" src={screenshot} alt={`Full-page screenshot of ${page.url}`} />{evidence.map((item, index) => <div key={`${item.finding.id}-${item.marker}-${index}`} className="annotation" style={{ left: `${item.x}%`, top: `${item.y}%`, width: `${item.width}%`, height: `${item.height}%` }}><span className="annotationMarker">{item.marker}</span></div>)}</div></div>
