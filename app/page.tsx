...
export default function Home() {
  ...
  const evidence = useMemo(() => page?.findings.flatMap((finding) => finding.evidence.map((item) => ({ ...item, findingId: finding.id }))) ?? [], [page]);
    ...
    <main className="shell">
      ...
      {result && page && (
        <section className="results" aria-live="polite">
          <div className="reportHeader"><div><div className="muted">AUDIT REPORT</div><h2>{result.pageTitle}</h2><a href={result.url} target="_blank" rel="noreferrer">{result.url}</a></div><div className="reportScore"><span>Site UX score</span><strong>{result.score}</strong><small>/100</small></div></div>
          <p className="reportSummary">{result.summary}</p>

          <div className="pageTabs" role="tablist" aria-label="Audited pages">
            {result.pages.map((item, index) => <button key={item.url} className={`pageTab ${selected === index ? "active" : ""}`} role="tab" aria-selected={selected === index} onClick={() => setSelected(index)}><span>{item.path === "/" ? "Home" : item.pageTitle || item.path}</span><small>{item.score}/100</small></button>)}
          </div>

          <div className="pageHeader"><div><div className="muted">PAGE {selected + 1} OF {result.pages.length}</div><h2>{page.pageTitle}</h2><a href={page.url} target="_blank" rel="noreferrer">{page.path}</a></div><div className="pageScore"><span>Page UX score</span><strong>{page.score}</strong><small>/100</small></div></div>
          <p className="reportSummary">{page.summary}</p>

          <div className="screenshotCard card">
            <div className="sectionHeader"><div><div className="muted">VISUAL EVIDENCE</div><h3>Page screenshot & issue markers</h3></div><span className="pill">{evidence.length} evidence marker{evidence.length === 1 ? "" : "s"}</span></div>
            <div className="visualEvidence">
              <div className="screenshotStage"><img src={page.screenshotUrl} alt={`Screenshot of ${page.url}`} />{evidence.map((item, index) => <div key={`${item.findingId}-${item.marker}-${index}`} className="annotation" style={{ left: `${item.x}%`, top: `${item.y}%`, width: `${item.width}%`, height: `${item.height}%` }}><span>{index + 1}</span></div>)}</div>
            ...
            <div className="evidenceLegend">Markers connect observed page evidence to the matching finding. Coordinates are approximate when inferred from HTML rather than a DOM screenshot.</div>
          </div>

          <div className="summary"><div className="card scoreCard"><div className="muted">PAGE SUMMARY</div><div className="metric">{page.score}/100</div><p>{page.summary}</p></div><div className="card"><div className="muted">High priority</div><div className="metric">{high}</div></div><div className="card"><div className="muted">Medium priority</div><div className="metric">{medium}</div></div><div className="card"><div className="muted">Findings</div><div className="metric">{page.findings.length}</div></div></div>
          ...
          <div className="findings">{page.findings.map((finding) => <article className="card finding" key={finding.id}>
            ...
            <div className="findingBody">
              ...
              <div className="detailGrid">
                <section className="detailBlock"><h4>ScreenRoot / UX tasks</h4><ul>{finding.screenrootTasks.map((task) => <li key={task}>{task}</li>)}</ul></section>
                <section className="detailBlock"><h4>Development tasks</h4><ul>{finding.devTasks.map((task) => <li key={task}>{task}</li>)}</ul></section>
                <section className="detailBlock lawBlock"><h4>UX law / principle</h4><div className="lawName">{finding.uxPerspective.law}</div><p><strong>Definition:</strong> {finding.uxPerspective.definition}</p><p><strong>Assessment:</strong> {finding.uxPerspective.assessment}</p></section>
              </div>
