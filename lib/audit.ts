...
function normalizeFinding(value: unknown, index: number, imageWidth: number, imageHeight: number): Finding {
...
async function captureScreenshot(url: string): Promise<Capture> {
  ...
  const code = `export default async ({ page }) => {
    ...
    try {
      await page.goto(${JSON.stringify(url)}, { waitUntil: "domcontentloaded", timeout: 9000 });
    } catch (error) {
      console.warn("Navigation did not finish before the navigation budget; capturing the rendered page", error);
    }
  ...
  return {
    buffer,
    width: Number(payload.width) || metadata.width || 1440,
    height: Number(payload.height) || metadata.height || 900,
  };
}
  ...
  for (const model of GEMINI_MODELS) {
    try {
      ...
      const findings = json.findings
        .map((item, index) => normalizeFinding(item, index, capture.width, capture.height))
...
function buildRegionSvg(width: number, height: number, findings: Finding[]): Buffer {
  const elements = findings.flatMap((finding) => finding.evidence.map((e) => {
    const x = clamp(e.x / 100 * width, 0, width - 1);
    const y = clamp(e.y / 100 * height, 0, height - 1);
    const w = clamp(e.width / 100 * width, 8, width - x);
    const h = clamp(e.height / 100 * height, 8, height - y);
    const markerW = 34;
    const markerH = 34;
    const markerX = x;
    const markerY = Math.max(0, y - markerH - 4);
...
async function renderRegions(mainImage: Buffer, width: number, height: number, findings: Finding[]): Promise<Buffer> {
  ...
  const overlay = buildRegionSvg(width, height, findings);
  return sharp(mainImage).composite([{ input: overlay, left: 0, top: 0, blend: "over" }]).png().toBuffer();
}
...
function applyVerificationBoxes(findings: Finding[], regions: VerificationResult["regions"]): Finding[] {
  const next = findings.map((finding) => ({ ...finding, evidence: finding.evidence.map((e) => ({ ...e })) }));
  for (const region of regions) {
    const finding = next.find((item) => item.id === region.findingId);
...
async function verifyAndCorrectRegions(capture: Capture, findings: Finding[], model: string): Promise<{ findings: Finding[]; annotated: Buffer }> {
  const apiKey = process.env.GEMINI_API_KEY;
...
export async function createAudit(url: string): Promise<AuditResult> {
  ...
  const verified = await verifyAndCorrectRegions(capture, analysis.findings, analysis.model);
  return {
    pages: [{
      url,
