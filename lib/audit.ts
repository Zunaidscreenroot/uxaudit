...
function normalizeFinding(value: unknown, index: number, imageWidth: number, imageHeight: number, textRegions: TextRegion[]): Finding {
  ...
  const evidenceBoxes = evidence.map((raw, evidenceIndex) => {
    ...
    if (anchored?.length) {
      ...
      const rawBox = Array.isArray(entry.box) ? entry.box.map(Number) : [];
      if (rawBox.length === 4 && rawBox.every(Number.isFinite)) {
        const [y1, x1, y2, x2] = rawBox.map((n) => clamp(n, 0, 1000));
    ...
    return {
      ...
      marker: typeof entry.marker === "string" ? entry.marker : String(index + 1),
      x: x / imageWidth * 100,
      y: y / imageHeight * 100,
  ...
  return {
    ...
    uxPerspective: {
      ...
      assessment: typeof perspective.assessment === "string" ? perspective.assessment : "This area deserves review based on the visible interface."
    },
    evidence: evidenceBoxes
  };
}
...
async function captureScreenshot(url: string): Promise<{ buffer: Buffer; width: number; height: number; textRegions: TextRegion[] }> {
  ...
  const response = await fetch(endpoint, {
    ...
    body: code,
    signal: AbortSignal.timeout(BROWSERLESS_TIMEOUT_MS + 1000)
  });
  if (!response.ok) {
    const detail = await response.text().catch(() => "");
    throw new Error(`Browserless returned HTTP ${response.status}${detail ? `: ${detail.slice(0, 300)}` : "."}`);
  }
  ...
  return {
    buffer: Buffer.from(payload.screenshot, "base64"),
    width: Number(payload.width) || 1440,
    height: Number(payload.height) || 900,
    textRegions: Array.isArray(payload.textRegions) ? payload.textRegions : []
  };
}
...
async function callGemini(apiKey: string, model: string, prompt: string, buffer: Buffer, maxOutputTokens: number): Promise<string> {
  ...
  if (!response.ok) {
    let message = `Gemini ${model} returned HTTP ${response.status}.`;
...
async function analyseWithGemini(url: string, title: string, capture: { buffer: Buffer; width: number; height: number; textRegions: TextRegion[] }): Promise<{ findings: Finding[]; model: string }> {
...
async function renderEvidenceVerificationScreenshot(capture: { buffer: Buffer; width: number; height: number }, findings: Finding[]): Promise<Buffer> {
  ...
  const regions = findings.flatMap((finding) => finding.evidence.map((evidence) => ({
    x: evidence.x,
    y: evidence.y,
  ...
  const endpoint = `https://production-sfo.browserless.io/function?token=${encodeURIComponent(process.env.BROWSERLESS_API_TOKEN || "")}&timeout=${BROWSERLESS_TIMEOUT_MS}`;
  const code = `export default async ({ page }) => {
    const screenshot = ${JSON.stringify(base64)};
    const regions = ${JSON.stringify(regions)};
    await page.setViewport({ width: ${capture.width}, height: 900, deviceScaleFactor: 1, isMobile: false, hasTouch: false });
    await page.setContent('<html><body style="margin:0;background:#fff"><img id="source" style="display:block;width:100%;height:auto" /></body></html>', { waitUntil: "domcontentloaded" });
    await page.evaluate((src) => { document.getElementById("source").src = src; }, 'data:image/png;base64,' + screenshot);
    await page.evaluate(() => new Promise(resolve => { const img = document.getElementById("source"); if (img.complete) resolve(true); else img.onload = () => resolve(true); }));
    await page.evaluate((items) => {
      ...
      for (const item of items) {
        ...
        box.style.cssText = `position:absolute;left:${item.x}%;top:${item.y}%;width:${item.width}%;height:${item.height}%;border:3px solid #ffd400;background:rgba(255,212,0,.12);box-sizing:border-box;font:700 16px Arial;color:#111;`;
        box.textContent = item.marker;
        overlay.appendChild(box);
      }
      document.body.appendChild(overlay);
      img.style.display = "block";
    }, regions);
    return await page.screenshot({ fullPage: true, type: "png", captureBeyondViewport: true, encoding: "base64" });
  };`;
  const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/javascript" }, body: code, signal: AbortSignal.timeout(BROWSERLESS_TIMEOUT_MS + 1000) });
  if (!response.ok) throw new Error(`Evidence verification render failed with HTTP ${response.status}.`);
  const payload = await response.json() as string | { screenshot?: string };
  const rendered = typeof payload === "string" ? payload : payload.screenshot;
  if (!rendered) throw new Error("Evidence verification render returned no screenshot.");
...
export async function createAudit(url: string): Promise<AuditResult> {
  ...
  const analysed = await analyseWithGemini(url, title, capture);
  const findings = await verifyEvidence(capture, analysed.findings, analysed.model);
  return {
    pages: [{
      url,
      title,
      screenshot: `data:image/png;base64,${capture.buffer.toString("base64")}`,
